...
<div class="companies container">
  <div class="row">
    <div class="col-md-6">
      <h2>اقســــام الطـــــــوارئ</h2>
    </div>
    <div class="col-md-6">
...
<br>
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                <th scope="row">{{ $num++ }}</th>
                <td> <a href="emergency/{{ $item->id }}">{{ $item->name }}</a>  </td>
                <td class="actions"> 
                  <a data-id="{{ $item->id }}" data-table="emergency_department" class="delete"><i class="far fa-trash-alt"></i></a>
                  <a data-id="{{ $item->id }}" class="edit_emergency_department" data-bs-toggle="modal" data-bs-target="#EditModal"><i class="far fa-pencil-alt"></i></a>
                </td>
              </tr>
            @endforeach
        </tbody>
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <form action="{{ route("store.emergency.department") }}" method="POST" class="modal-content" enctype="multipart/form-data" >
        @csrf
  ...
  <div class="modal fade" id="EditModal" tabindex="-1" aria-labelledby="EditModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <form action="{{ route("update.emergency.department") }}" method="POST" class="modal-content" enctype="multipart/form-data" >
        @csrf
        <div class="modal-header">
...
<script>
        $(document).on("click", ".edit_emergency_department", function(event) {
          event.preventDefault()
          var id     = $(this).data("id");
        $.ajax({
          url: "/ajax.get.emergency.department",
          data:{id:id },
