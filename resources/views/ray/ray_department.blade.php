...
<br>
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                <th scope="row">{{ $num++ }}</th>
                <td> <a href="ray/{{ $item->id }}">{{ $item->name }}</a>  </td>
                <td class="actions"> 
                  <a data-id="{{ $item->id }}" data-table="ray_department" class="delete"><i class="far fa-trash-alt"></i></a>
                  <a data-id="{{ $item->id }}" class="edit_ray_department" data-bs-toggle="modal" data-bs-target="#EditModal"><i class="far fa-pencil-alt"></i></a>
                </td>
              </tr>
            @endforeach
        </tbody>
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <form action="{{ route("store.ray.department") }}" method="POST" class="modal-content" enctype="multipart/form-data" >
        @csrf
...
<script>
        $(document).on("click", ".edit_ray_department", function(event) {
          event.preventDefault()
          var id     = $(this).data("id");
        $.ajax({
          url: "/ajax.get.ray.department",
          data:{id:id },
