...
<div class="companies container">
  <div class="row">
    <div class="col-md-6">
      <h2>اقســـام العلاج الطبيعـــي</h2>
    </div>
    <div class="col-md-6">
...
<br>
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                <th scope="row">{{ $num++ }}</th>
                <td> <a href="physical.therapy/{{ $item->id }}">{{ $item->name }}</a>  </td>
                <td class="actions"> 
                  <a data-id="{{ $item->id }}" data-table="physical_therapy_department" class="delete"><i class="far fa-trash-alt"></i></a>
                  <a data-id="{{ $item->id }}" class="edit_physical_therapy_department" data-bs-toggle="modal" data-bs-target="#EditModal"><i class="far fa-pencil-alt"></i></a>
                </td>
              </tr>
            @endforeach
        </tbody>
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <form action="{{ route("store.physical.therapy.department") }}" method="POST" class="modal-content" enctype="multipart/form-data" >
        @csrf
  ...
  <div class="modal fade" id="EditModal" tabindex="-1" aria-labelledby="EditModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <form action="{{ route("update.ray.department") }}" method="POST" class="modal-content" enctype="multipart/form-data" >
        @csrf
        <div class="modal-header">
...
<script>
$(document).on("click", ".edit_physical_therapy_department", function(event) {
          event.preventDefault()
          var id     = $(this).data("id");
        $.ajax({
          url: "/ajax.get.physical.therapy.department",
          data:{id:id },
