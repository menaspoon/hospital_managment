...
use Hash;
use DB;
class RayDepartmentController extends Controller {

  function index() {
      $acount = User::where("id", Auth::id())->first();
      $categories  = RayDepartment::where("hospital", $acount->hospital)->get();
      return view("ray.ray_department", compact("categories"));
  }
  ...
  function edit(Request $request) {
    if($request->ajax()) {
      $id = $request->get('id');
      $data = RayDepartment::where('id', $id)->first();
  ...
  function  update(Request $request) {
    ...
    RayDepartment::where("id", $request->id)->update([
