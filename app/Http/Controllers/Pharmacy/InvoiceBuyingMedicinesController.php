...
use App\Models\Pharmacy\InvoiceBuyingMedicines;
use App\Models\Pharmacy\InvoiceDetalesBuyingMedicines;
...
class InvoiceBuyingMedicinesController extends Controller {


  function index() {
      $acount = User::where("id", Auth::id())->first();
      $purchases  = InvoiceBuyingMedicines::where("hospital", $acount->hospital)->paginate();
      return view("pharmacy.buying_medicines", compact("purchases"));
  }
  ...
  function create() {
      ...
      $pharmacy  = Pharmacy::where("hospital", $acount->hospital)->get();
      return view("pharmacy.new_buying_medicines", compact("pharmacy", "banks", "supplier"));
  }
    ...
    function  store(Request $request) {
      ...
      $purchases = InvoiceBuyingMedicines
      ...
      for ($i = 0; $i < count($request->product); $i++) { 
          ...
          InvoiceDetalesBuyingMedicines::insert($InvoiceDetales);
      }

      // تحديد عدد المنتجات
      if ($request->invoice_type == "purchases") :
          for ($i = 0; $i < count($request->product); $i++) { 
              $count = Pharmacy::where("id", $request->product[$i])->first();
              Pharmacy::where('id', $request->product[$i])->update([
